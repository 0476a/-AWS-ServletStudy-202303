...
// 요청을 받는 페이지의 경로를 설정해줌. ("*"은 모든 요청을 뜻함.)
@WebFilter("/*")
public class AuthFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// sesstion 시간 설정 가능 기본값은 30분
//		httpRequest.getSession().setMaxInactiveInterval(60*60*24*365);
		...
		for(String antMatcher : antMatchers) {
			if(uri.startsWith(antMatcher)) {
				...
				if(session.getAttribute("AuthenticationPrincipal") == null) {
					// 인증을 받지 않으면 로그인 페이지로 보내버림. 요청이 되어야만 url에 들어갈수 있음.
